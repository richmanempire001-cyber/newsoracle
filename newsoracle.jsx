...
function Ticker({ posts }) {
  const items = posts.length > 0
    ? posts.map(p => `${p.tag.toUpperCase()}: ${p.title}`)
    : ["NEWSORACLE — Live sports and financial analysis", "Markets coverage across stocks, crypto and forex", "Sports predictions and match previews updated throughout the day"];
...
function ArticleCard({ post, size = "normal" }) {
  ...
  if (size === "hero") return (
    <div style={{ borderBottom: "1px solid #e5e7eb", paddingBottom: 28, marginBottom: 28 }}>
      ...
      <h2 onClick={() => setOpen(!open)} style={{ fontFamily: "'Georgia', serif", fontSize: 28, fontWeight: 700, color: "#111827", margin: "0 0 12px", lineHeight: 1.3, cursor: "pointer", letterSpacing: "-0.3px" }}>{post.title}</h2>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
        ...
        {post.prediction && <button onClick={() => setOpen(!open)} style={{ background: "none", border: "1px solid #d1d5db", color: "#374151", fontSize: 12, padding: "5px 12px", cursor: "pointer", borderRadius: 2 }}>{open ? "Hide analysis" : "View analysis"}</button>}
  ...
  if (size === "small") return (
    <div style={{ paddingBottom: 16, marginBottom: 16, borderBottom: "1px solid #f3f4f6" }}>
      <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
        <span style={{ background: catColor, color: "#fff", fontSize: 9, fontWeight: 800, padding: "2px 6px", letterSpacing: "0.08em" }}>{post.category.toUpperCase()}</span>
  ...
  return (
    <div style={{ paddingBottom: 20, marginBottom: 20, borderBottom: "1px solid #e5e7eb" }}>
      ...
      {open && post.prediction && (
        <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderLeft: "3px solid #1a56db", padding: 14, margin: "10px 0", borderRadius: "0 4px 4px 0" }}>
          <div style={{ color: "#1a56db", fontSize: 11, fontWeight: 800, letterSpacing: "0.1em", marginBottom: 6 }}>ANALYSIS & OUTLOOK</div>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <a href={post.link} target="_blank" rel="noreferrer" style={{ color: "#1a56db", fontSize: 13, fontWeight: 600, textDecoration: "none" }}>Read full story →</a>
        {post.prediction && <button onClick={() => setOpen(!open)} style={{ background: "none", border: "1px solid #d1d5db", color: "#374151", fontSize: 12, padding: "4px 10px", cursor: "pointer", borderRadius: 2 }}>{open ? "Close" : "View analysis"}</button>}
      </div>
    </div>
  );
}
...
// ── MAIN ───────────────────────────────────────────────────────────────────
export default function NewsOracle() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    } catch { setError("Could not fetch news. Check your connection."); }
    setFetching(false);
  }, []);
...
{
  "summary": "3 clear, factual sentences summarising the story in journalistic style. Third person, active voice, no hype.",
  "prediction": "${isSports ? "2 sentences analysing likely outcomes based on current form, statistics, and context. Be specific — mention teams, players, or likely scores where relevant." : "2 sentences of forward-looking market analysis. Reference specific factors driving the outlook. Mention percentage moves or price levels where relevant."}",
  ...
  "sentiment": "${isSports ? "home_win OR away_win OR draw OR bullish OR bearish" : "bullish OR bearish OR neutral"}"
      ...
      setPosts(p => [{
        ...
        summary: parsed.summary, prediction: parsed.prediction,
        confidence: parsed.confidence, sentiment: parsed.sentiment,
        source: article.source, tag: article.tag,
        category: article.category, link: article.link,
        pubDate: article.pubDate, postedAt: new Date().toISOString(),
  ...
  const postAll = useCallback(async () => {
    ...
    for (const a of unposted) { if (autoRef.current || true) await processArticle(a); }
  ...
  useEffect(() => {
    if (autoMode) {
      fetchAll();
      const i = setInterval(fetchAll, 30 * 60 * 1000);
      return () => clearInterval(i);
    }
  }, [autoMode, fetchAll]);

  useEffect(() => {
    if (autoMode && articles.length > 0) postAll();
  }, [articles]);

  const tabs = ["All", "Sports", "Markets", "Crypto"];

  const filteredSite = posts.filter(p =>
    siteTab === "All" ? true :
    siteTab === "Sports" ? p.category === "Sports" :
    siteTab === "Markets" ? p.category === "Markets" :
    p.category === "Crypto"
  );

  const filteredDash = posts.filter(p =>
    activeTab === "All" ? true : p.category === activeTab
  );

  // ── SETUP ──────────────────────────────────────────────────────────────
  if (screen === "setup") return (
    <div style={{ minHeight: "100vh", background: "#f9fafb", fontFamily: font, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ width: "100%", maxWidth: 440 }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ fontFamily: serif, fontSize: 38, fontWeight: 700, color: "#111827", letterSpacing: "-1px", marginBottom: 4 }}>NewsOracle</div>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 4, padding: 32, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          <input type="password" value={apiInput} onChange={e => setApiInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleKey()} placeholder="sk-ant-api03-..."
            style={{ width: "100%", marginTop: 8, padding: "12px 14px", border: "1px solid #d1d5db", borderRadius: 3, fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: font, color: "#111" }} />
          {keyError && <p style={{ color: "#CC0000", fontSize: 12, marginTop: 6 }}>{keyError}</p>}
          <button onClick={handleKey}
            style={{ width: "100%", marginTop: 16, padding: "12px", background: "#111827", border: "none", borderRadius: 3, color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", letterSpacing: "0.01em" }}>
            Enter Newsroom
          </button>
    ...
    <div style={{ minHeight: "100vh", background: "#fff", fontFamily: font }}>
      ...
      <div style={{ background: "#111827", padding: "0 32px", height: 36, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          {["Sports", "Markets", "Crypto", "Analysis"].map(l => (
      ...
      <div style={{ borderBottom: "3px solid #111827", padding: "20px 32px 16px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", borderBottom: "1px solid #e5e7eb", paddingBottom: 16, marginBottom: 16 }}>
            <div style={{ fontFamily: serif, fontSize: 52, fontWeight: 700, color: "#111827", letterSpacing: "-2px", lineHeight: 1 }}>NewsOracle</div>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 4 }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setSiteTab(t)}
                style={{ background: siteTab === t ? "#111827" : "transparent", color: siteTab === t ? "#fff" : "#374151", border: "none", padding: "6px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", letterSpacing: "0.03em" }}>
                {t}
              </button>
            ))}
            <button onClick={() => setScreen("dashboard")}
              style={{ background: "#CC0000", color: "#fff", border: "none", padding: "6px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", marginLeft: 8 }}>
              Newsroom
            </button>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 32px" }}>
        {filteredSite.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 0", borderTop: "1px solid #e5e7eb" }}>
            <div style={{ fontFamily: serif, fontSize: 24, color: "#9ca3af", marginBottom: 12 }}>No stories published yet</div>
            <p style={{ color: "#9ca3af", fontSize: 14 }}>Open the Newsroom to fetch and publish stories.</p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 48 }}>
            ...
            <div style={{ borderLeft: "1px solid #e5e7eb", paddingLeft: 32 }}>
              <div style={{ borderBottom: "2px solid #111827", paddingBottom: 8, marginBottom: 16 }}>
                <span style={{ fontFamily: serif, fontSize: 16, fontWeight: 700, color: "#111827" }}>Latest</span>
              </div>
              ...
              <div style={{ borderTop: "2px solid #111827", paddingTop: 16, marginTop: 8 }}>
                <div style={{ fontFamily: serif, fontSize: 16, fontWeight: 700, color: "#111827", marginBottom: 14 }}>At a Glance</div>
                {[
                  { label: "Stories Published", val: posts.length },
                  { label: "Sports Coverage", val: posts.filter(p=>p.category==="Sports").length },
                  { label: "Markets Coverage", val: posts.filter(p=>p.category==="Markets").length },
                  { label: "Crypto Coverage", val: posts.filter(p=>p.category==="Crypto").length },
                ].map(({ label, val }) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #f3f4f6" }}>
                    ...
                    <span style={{ color: "#111827", fontWeight: 700, fontSize: 13 }}>{val}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <footer style={{ borderTop: "3px solid #111827", background: "#111827", padding: "32px", marginTop: 48 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ fontFamily: serif, fontSize: 24, color: "#fff", marginBottom: 8 }}>NewsOracle</div>
          <p style={{ color: "#6b7280", fontSize: 12, maxWidth: 700, lineHeight: 1.6, margin: 0 }}>
            All analysis and forward-looking statements on NewsOracle are provided for informational purposes only. They do not constitute financial, investment, or betting advice. NewsOracle accepts no liability for decisions made on the basis of content published on this site. © {new Date().getFullYear()} NewsOracle. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
    ...
    <div style={{ minHeight: "100vh", background: "#f3f4f6", fontFamily: font }}>
      ...
      <div style={{ background: "#111827", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 56, borderBottom: "3px solid #CC0000" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <span style={{ fontFamily: serif, color: "#fff", fontWeight: 700, fontSize: 20, letterSpacing: "-0.5px" }}>NewsOracle</span>
          <span style={{ color: "#6b7280", fontSize: 13 }}>/ Newsroom</span>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button onClick={() => setScreen("site")}
            style={{ background: "transparent", border: "1px solid #374151", color: "#d1d5db", padding: "7px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", borderRadius: 2 }}>
            View Site ({posts.length})
          </button>
          <div onClick={() => setAutoMode(!autoMode)}
            style={{ display: "flex", alignItems: "center", gap: 8, background: autoMode ? "#064e3b" : "#1f2937", border: `1px solid ${autoMode ? "#10b981" : "#374151"}`, padding: "7px 16px", cursor: "pointer", borderRadius: 2 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: autoMode ? "#10b981" : "#4b5563", boxShadow: autoMode ? "0 0 6px #10b981" : "none" }} />
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "28px 32px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
            {[
              { label: "Fetched", val: articles.length, color: "#1a56db" },
              { label: "Published", val: posts.length, color: "#166534" },
              { label: "Sports", val: posts.filter(p=>p.category==="Sports").length, color: "#1a56db" },
              { label: "Markets", val: posts.filter(p=>p.category!=="Sports").length, color: "#b45309" },
            ].map(s => (
              <div key={s.label} style={{ background: "#fff", border: "1px solid #e5e7eb", padding: "14px 16px", borderTop: `3px solid ${s.color}` }}>
                <div style={{ color: "#6b7280", fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>{s.label}</div>
                <div style={{ color: "#111827", fontSize: 24, fontWeight: 800, marginTop: 2 }}>{s.val}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", padding: 20, marginBottom: 20 }}>
            <div style={{ fontFamily: serif, fontSize: 16, fontWeight: 700, color: "#111827", marginBottom: 8, borderBottom: "2px solid #111827", paddingBottom: 8 }}>Fetch News</div>
            ...
            {error && <div style={{ background: "#fef2f2", border: "1px solid #fecaca", padding: 10, marginBottom: 12, borderRadius: 2 }}><p style={{ color: "#CC0000", fontSize: 13, margin: 0 }}>{error}</p></div>}
            <button onClick={fetchAll} disabled={fetching}
              style={{ width: "100%", padding: "11px", background: fetching ? "#f3f4f6" : "#111827", border: "none", color: fetching ? "#9ca3af" : "#fff", fontSize: 14, fontWeight: 600, cursor: fetching ? "not-allowed" : "pointer" }}>
              {fetching ? "Fetching…" : "Fetch Latest Stories"}
            </button>
          </div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb" }}>
            <div style={{ padding: "14px 20px", borderBottom: "2px solid #111827", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontFamily: serif, fontSize: 16, fontWeight: 700, color: "#111827" }}>Articles</span>
              {articles.length > 0 && (
                <button onClick={postAll} disabled={!!processing}
                  style={{ background: "#CC0000", border: "none", color: "#fff", fontSize: 12, fontWeight: 700, padding: "6px 14px", cursor: processing ? "not-allowed" : "pointer" }}>
                  {processing ? "Publishing…" : `Publish All (${articles.filter(a => !posts.find(p => p.id === a.id)).length})`}
                </button>
              )}
            </div>
            {articles.length === 0 ? (
              <div style={{ padding: "48px 20px", textAlign: "center", color: "#9ca3af" }}>
                <div style={{ fontFamily: serif, fontSize: 18, marginBottom: 8 }}>No articles fetched yet</div>
                <p style={{ fontSize: 13, margin: 0 }}>Click "Fetch Latest Stories" above</p>
              </div>
            ) : (
                    ...
                    <div key={article.id} style={{ padding: "14px 20px", borderBottom: "1px solid #f3f4f6", display: "flex", gap: 12, alignItems: "flex-start", background: isPosted ? "#f0fdf4" : "#fff" }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", gap: 6, marginBottom: 4, flexWrap: "wrap" }}>
                          <span style={{ background: catColor, color: "#fff", fontSize: 9, fontWeight: 800, padding: "2px 6px", letterSpacing: "0.08em" }}>{article.category.toUpperCase()}</span>
                      ...
                      <button onClick={() => processArticle(article)} disabled={!!isPosted || isProcessing}
                        style={{ flexShrink: 0, padding: "6px 12px", border: "none", fontSize: 11, fontWeight: 700, cursor: isPosted ? "default" : "pointer",
                          background: isPosted ? "#dcfce7" : isProcessing ? "#f3f4f6" : "#111827",
                          color: isPosted ? "#166534" : isProcessing ? "#9ca3af" : "#fff" }}>
        ...
        <div>
          {/* Tab bar */}
          <div style={{ display: "flex", gap: 0, marginBottom: 0, borderBottom: "2px solid #111827" }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setActiveTab(t)}
                style={{ background: activeTab === t ? "#111827" : "#fff", color: activeTab === t ? "#fff" : "#374151", border: "1px solid #e5e7eb", borderBottom: "none", padding: "8px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                {t}
              </button>
            ))}
          </div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderTop: "none", maxHeight: 460, overflowY: "auto" }}>
            {filteredDash.length === 0 ? (
              <div style={{ padding: "48px 20px", textAlign: "center", color: "#9ca3af" }}>
                <div style={{ fontFamily: serif, fontSize: 18, marginBottom: 8 }}>Nothing published yet</div>
                <p style={{ fontSize: 13, margin: 0 }}>Fetch articles and hit Publish</p>
              </div>
              ...
              <div key={post.id} style={{ padding: "16px 20px", borderBottom: "1px solid #f3f4f6" }}>
                ...
                <p style={{ fontFamily: serif, color: "#111827", fontSize: 14, fontWeight: 700, margin: "0 0 6px", lineHeight: 1.4 }}>{post.title}</p>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", marginTop: 20, padding: 20 }}>
            <div style={{ fontFamily: serif, fontSize: 16, fontWeight: 700, color: "#111827", borderBottom: "2px solid #111827", paddingBottom: 8, marginBottom: 14 }}>Auto Mode</div>
            <p style={{ color: "#6b7280", fontSize: 13, lineHeight: 1.6, margin: "0 0 14px" }}>
              When enabled, the newsroom fetches and publishes new stories automatically every 30 minutes — around the clock, without any manual input.
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              {[["30 min", "refresh cycle"], ["7 sources", "news feeds"], ["24/7", "unattended"]].map(([v, l]) => (
          ...
          {/* Log */}
          <div style={{ background: "#111827", border: "1px solid #374151", marginTop: 16, padding: 16 }}>
            ...
            <div style={{ maxHeight: 140, overflowY: "auto" }}>
              {log.length === 0
                ? <p style={{ color: "#4b5563", fontSize: 12, margin: 0 }}>No activity yet</p>
                : log.map((l, i) => <p key={i} style={{ color: i === 0 ? "#10b981" : "#6b7280", fontSize: 12, margin: "0 0 4px", fontFamily: "monospace" }}>{l}</p>)
              }
            </div>
          </div>
        </div>
